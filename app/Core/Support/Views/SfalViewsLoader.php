...
namespace SFAL\Core\Support\Views;

defined('ABSPATH') || exit('no access');

final class SfalViewsLoader
{
    ...
    private function validatePath(string $path)
    {
        if (! $this->isValidPath($path)) {
           /* translators: %1$s is replaced with "view path" */
            wp_die(sprintf(__('the view path : %s to be loaded is not exists', 'sfal'), $path));
        }
    }
     ...
     * @param  string  $path
     *
     * @return \SFAL\Core\Support\Views\SfalViewsLoader
     */
    public static function getInstance(string $path): self
    {
        null === self::$instance && self::$instance = new static($path);
