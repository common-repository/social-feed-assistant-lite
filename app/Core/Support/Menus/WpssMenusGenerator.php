...
class WpssMenusGenerator
{
    ...
    {
        self::$prefix   = $prefix;
        self::$menus    = apply_filters('WpssAdminMenus', WpssConfig('pages.social-stream'));
        self::$submenus = apply_filters('WpssAdminSubmenus', WpssConfig('subpages.social-stream'));
    ...
    private static function requiredKey(string $key, array $menu)
    {
        if (! array_key_exists($key, $menu)) {
            wp_die(__(sprintf('the %s key is required !', $key), 'wp-ss'));
        }
    }
}
