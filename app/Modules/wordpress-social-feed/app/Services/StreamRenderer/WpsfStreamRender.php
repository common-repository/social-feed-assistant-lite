...
namespace WPSF\Services\StreamRenderer;

defined('ABSPATH') || exit('no access');

use Tightenco\Collect\Support\Collection;
use WPSF\Services\StreamRenderer\Presenters\WpsfStreamClassicTemplateHandler;
use WPSF\Services\StreamRenderer\Presenters\WpsfStreamTileTemplateHandler;
use WPSF\Services\WpsfStreamsService;

class WpsfStreamRender
{
    const TEMPLATES = ['classic' => WpsfStreamClassicTemplateHandler::class, 'tile' => WpsfStreamTileTemplateHandler::class];
    ...
    {
        self::$stream = WpsfStreamsService::getStream($streamID, [], ['options', 'relatedOptions']);
        self::$feeds = WpsfStreamsService::getStreamFeedIds($streamID);

        self::$posts = WpsfStreamsService::getStreamPosts($streamID, self::$feeds);
    ...
    private static function sortAndPaginatePosts(Collection &$posts, int $itemsOrder = 3)
    {
        ...
        if (array_key_exists($itemsOrder, $sortables)) {
            self::{$sortables[ $itemsOrder ]}($posts);
        }
    }
    ...
    private static function sortBySourceListPosition(Collection &$posts)
    {
        ...
        usort($posts, function ($postOne, $postTwo) use ($sources) {
            ...
            return $postOne->feed_id == $postTwo->feed_id ? $postOne->feed_multi > $postTwo->feed_multi : $res;
        });
