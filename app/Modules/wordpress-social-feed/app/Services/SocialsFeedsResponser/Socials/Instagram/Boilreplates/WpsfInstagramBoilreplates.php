...
namespace WPSF\Services\SocialsFeedsResponser\Socials\Instagram\Boilreplates;
...
use WPSF\Services\SocialsFeedsResponser\Utils\WpsfFeedGeneralUtils;

class WpsfInstagramBoilreplates
{
    ...
    public static function userBoilreplate(Account $account) : array
    {
        $b = [
            ...
            'fullname'   => WpsfFeedGeneralUtils::removeEmoji($account->getFullName()),
            'id'         => $account->getId(),
    ...
    public static function postBoilreplate(Media $post, array $user)
    {
        $b = [
            ...
            'images'     => [
                ...
                'standard'  => $post->getImageStandardResolutionUrl(),
                'high'      => $post->getImageHighResolutionUrl(),
            ],
            'videos' => [
                'low'       => $post->getVideoLowResolutionUrl(),
                'standard'  => $post->getVideoStandardResolutionUrl(),
    ...
    {
        if (array_key_exists($b['media_type'], self::MEDIA_SETTER)) {
            self::{self::MEDIA_SETTER[$b['media_type']]}($b, $post);
        }
    }
    ...
    private static function createMedia(Media $media)
    {
        if (Media::TYPE_VIDEO == $media->getType() && $media->getVideoStandardResolutionUrl()) {
            return ['type' => Media::TYPE_VIDEO, 'url' => [
                ...
                'standard'  => $media->getVideoStandardResolutionUrl(),
